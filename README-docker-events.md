...
|---|---|---|
| Vast (CPU mining, testing) | `source/no-container-docker_events_monitor-vast.sh` | `vast` |
| Nosana / Podman (testing) | `nosana_monitor-1.sh`, `source/podman_events_monitor.sh` | `podman` |

> Note: a short server test seems to run around benchmark time — likely the platform confirming host specs.
