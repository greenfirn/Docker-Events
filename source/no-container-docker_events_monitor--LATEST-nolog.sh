...
start_miner() {
    ...
    # Start in screen session - output stays in the screen session's own
    # scrollback (view live with `screen -r $SCREEN_NAME`), no longer
    # teed to a log file on disk. Nothing reads that file anymore: every
    # miner's stats now come from its own API (keryx-miner included, as
    # of its 0.3.7 stats API), so the log-tail/trim machinery this used to
    # need was dead weight.
